...
export default function TestSeriesPage({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchTestAttempt = async () => {
      try {
        await dispatch(testSeriesActions.fetchTestSeriesOnIdSlice(id)).unwrap();
        
        // Check if a test attempt already exists
        /*const existingTestAttempt = await dispatch(
          testAttemptModelActions.getTestAttemptByUserAndTest({ 
            userId: session.user.id, 
            testseriesId: id 
          })
        ).unwrap();

        console.log('existingTestAttempt:',existingTestAttempt);
