...
const SubjectImportExport = () => {
  const [isProcessing, setIsProcessing] = useState(false);
  const [importSuccess, setImportSuccess] = useState(false);
  const [importError, setImportError] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const [subjects, setSubjects] = useState([]); // Store questions data
  ...
  const handleExport = () => {
    ...
    const csv = Papa.unparse(subjectModels); // Convert questions to CSV format
        ...
        fileReader.onload = () => {
          ...
          Papa.parse(content, {
            complete: (result) => {
              ...
              setSubjects(transformedData); // Set the imported questions in state
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const action = dispatch(subjectModelActions.createSubjectModelSlice({ subjects }));
      setSubjects([]);
  ...
  return (
    <ProtectedRoute roleRequired={'ADMIN'}>
      ...
      <div className="mb-6">
        <label className="block text-gray-700">Import Subjects</label>
        <div 
        ...
        >
          ...
          {importSuccess && <div className="mt-2 text-green-500">Subjects imported successfully!</div>}
          {importError && <div className="mt-2 text-red-500">There was an error with the file. Please try again.</div>}
        </div>
        
        <div className='flex justify-center items-center h-full p-4'>
          <Button  onClick={handleSubmit} variant="default">Submit Subjects</Button>
        </div>
...
export default SubjectImportExport;
