...
const AddExams = () =>{
  ...
  const handleSubmit =async(e)=>{
    e.preventDefault();
    try{
      dispatch(examModelActions.createExamModelSlice({'exams':[{name,description}]}))
      setName('');
      setDescription('');
      setIsLoading(false);
    }catch(error){
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 mt-10">
            <div className="space-y-2">
              <label htmlFor="name" className="text-md font-medium block">Exam Name</label>
              <input 
            ...
            <div className="">
            <button type="submit" className="px-5 py-3 mt-4 text-md font-medium bg-blue-700 text-white rounded-xl hover:bg-blue-600">{isLoading ? 'Creating...' : 'Create Exam'}</button>
            </div>
