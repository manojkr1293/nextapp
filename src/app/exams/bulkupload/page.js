...
const ExamsImportExport = () => {
  const [isProcessing, setIsProcessing] = useState(false);
  const [importSuccess, setImportSuccess] = useState(false);
  const [importError, setImportError] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const [exams, setExams] = useState([]); // Store questions data
  ...
  const handleExport = () => {
    ...
    const csv = Papa.unparse(examModels); // Convert questions to CSV format
        ...
        fileReader.onload = () => {
          ...
          Papa.parse(content, {
            complete: (result) => {
              ...
              setExams(transformedData); // Set the imported questions in state
  ...
  const handleSubmit = async () => {
    setIsProcessing(true);
    if (exams.length === 0) {
      alert('No exams to submit');
      return;
    }

    try {
      console.log('exams:',exams);
      const action = dispatch(examModelActions.createExamModelSlice({ exams }));
      setExams([]);
  ...
  return (
    <ProtectedRoute roleRequired={'ADMIN'}>
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold">Manage Exam</h2>
        <button
      ...
      <div className="mb-6">
        <label className="block text-gray-700">Import Exams</label>
        <div 
        ...
        >
          ...
          {importSuccess && <div className="mt-2 text-green-500">Classs imported successfully!</div>}
          {importError && <div className="mt-2 text-red-500">There was an error with the file. Please try again.</div>}
        </div>
        
        <div className='flex justify-center items-center h-full p-4'>
          <Button  onClick={handleSubmit} variant="default">Submit Exam</Button>
        </div>
...
export default ExamsImportExport;
