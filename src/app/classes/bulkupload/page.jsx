...
const ClassImportExport = () => {
  const [isProcessing, setIsProcessing] = useState(false);
  const [importSuccess, setImportSuccess] = useState(false);
  const [importError, setImportError] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const [classes, setClasses] = useState([]); // Store questions data
  ...
  const handleExport = () => {
    ...
    const csv = Papa.unparse(classModels); // Convert questions to CSV format
        ...
        fileReader.onload = () => {
          ...
          Papa.parse(content, {
            complete: (result) => {
              ...
              setClasses(transformedData); // Set the imported questions in state
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const action = dispatch(classModelActions.createClassModelSlice({ classes }));
      setClasses([]);
  ...
  return (
    <ProtectedRoute roleRequired={'ADMIN'}>
      ...
      <div className="mb-6">
        <label className="block text-gray-700">Import Class</label>
        <div 
        ...
        <div className='flex justify-center items-center h-full p-4'>
          <Button  onClick={handleSubmit} variant="default">Submit Class</Button>
        </div>
...
export default ClassImportExport;
