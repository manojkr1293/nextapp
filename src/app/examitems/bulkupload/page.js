...
const ExamItemsImportExport = () => {
  const [isProcessing, setIsProcessing] = useState(false);
  const [importSuccess, setImportSuccess] = useState(false);
  const [importError, setImportError] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const [examitems, setExamItems] = useState([]); // Store questions data
  ...
  const handleExport = () => {
    ...
    const csv = Papa.unparse(examItemModels); // Convert questions to CSV format
    const blob = new Blob([csv], { type: 'text/csv' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'examItems.csv';
    link.click();
  };
        ...
        fileReader.onload = () => {
          ...
          Papa.parse(content, {
            complete: (result) => {
              ...
              setExamItems(transformedData); // Set the imported questions in state
  ...
  const handleSubmit = async () => {
    ...
    if (examitems.length === 0) {
      alert('No Exam Items to submit');
      return;
    }

    try {
      console.log('Exam Items:',examitems);
      const action = dispatch(examItemModelActions.createexamItemModelSlice({ examitems }));
      setExamItems([]);
  ...
  return (
    <ProtectedRoute roleRequired={'ADMIN'}>
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold">Manage Exam Items</h2>
        <button
          onClick={handleExport}
          className="bg-green-500 text-white p-2 rounded-md hover:bg-green-600"
        >
          Export Exam Items (CSV)
        </button>
      </div>
      ...
      <div className="mb-6">
        <label className="block text-gray-700">Import Exam Items</label>
        <div 
        ...
        >
          ...
          {importSuccess && <div className="mt-2 text-green-500">Exam Items imported successfully!</div>}
          {importError && <div className="mt-2 text-red-500">There was an error with the file. Please try again.</div>}
        </div>
        
        <div className='flex justify-center items-center h-full p-4'>
          <Button  onClick={handleSubmit} variant="default">Submit Exam Items</Button>
        </div>
...
export default ExamItemsImportExport;
